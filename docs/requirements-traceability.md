...
| --- | --- | --- |
| הצגת כתבות שאושרו בלבד | סינון לפי `publishedVersion.publishedAt` ושימוש ב-`publishedVersion` בלבד, גם כאשר עדכון חדש נמצא בבדיקה | בדיקת בקר שמוודאת שאין דליפת `workingVersion` ושהגרסה המאושרת נשארת בפיד |
